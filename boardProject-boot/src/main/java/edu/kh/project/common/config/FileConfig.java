...
public class FileConfig implements WebMvcConfigurer{
	...
	// WebMvcConfigurer : Spring MVC 프레임워크에서 제공하는 인터페이스중 하나로,
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		// Spring MVC 에서 정적 리소스(이미지, CSS, JS 파일 등)의 요청을 처리하기 위해 사용하는 클래스
	...
	public MultipartConfigElement configElement() {
		...
		// 업로드 파일의 최대크기, 메모리에서의 임시 저장 경로등 설정 저장 가능
	...
	// MultipartResolver 객체를 생성하여 Bean으로 등록하는 메서드
	// -> Bean으로 등록하면서, MultipartConfigElement 를 자동으로 이용함
	
	@Bean
	public MultipartResolver MultipartResolver() {
		// MultipartResolver : MultipartFile 을 처리해주는 해결사
		// MultipartResolver는 클라이언트로 받은 multipart 요청을 처리하고,
		// 이 중 업로도된 파일을 추출하여 MultipartFile 객체로 제공하는 역할
