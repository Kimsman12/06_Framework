...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        if(currentNickname !== memberNickname.value) {
            ...
            // await : 비동기 작업의 결과를 기다릴때 사용하는 키워드 -> !!! 반드시 async 함수 안에서만 사용가능
            //      -> "이 작업이 끝날때 까지 기다려주세요."
...
// 이미지 선택시 미리보기 및 파일 크기 검사
imageInput.addEventListener("change", ()=>{
    // change 이벤트 : 기존에 있던 값과 달라지면 change이벤트
    ...
    if(file) { // 파일이 선택된 경우
        if(file.size <= MAX_SIZE) { // 파일 크기가 허용범위 이내인 경우
            ...
            imageInput.value="";
            // 1. 파일 선택 초기화 (alert 창은 띄웠지만 이미 선택된 큰사이즈 파일을 비우는건 따로 처리)
            // == imageInput.files = null;
            
            // 2. 이전 미리보기 이미지로 복원
            profileImg.src=previousImage; 

            // 3.파일 입력 복구 : 이전 파일이 존재 하면 다시 할당
            if(previousFile){
                const dataTransfer = new DataTransfer();
                // DataTransfer : JS로 파일을 조작할때 사용되는 인터페이스
                ...
                // DataTransfer를 이용하여 현재 File객체를 FileList 반환하여 할당
                
                // 이전 파일 추가 : DataTransfer 에 File 객체 추가
                dataTransfer.items.add(previousFile);
                
                // 이전 파일로 input 요소의 files 속성 복구 : DataTransfer에 저장된 파일의 리스트를 FileList 객체로 반환
        ...
        profileImg.src = previousImage; // 이전 미리보기 이미지 복원
        ...
        if(previousFile) {
            ...
            dataTransfer.items.add(previousFile);
            imageInput.files = DataTransfer.files; // 이전 파일로 input태그의 files 속성 복구
        }
    }
});
